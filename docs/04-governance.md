...
- **Secrets**: never in the repo (enforced by `.gitignore` + secret-scanning in CI). Agents receive scoped tokens (fine-grained GitHub PAT per purpose), rotated on schedule. Agents never see deploy credentials — deployment runs in CI with GitHub-managed secrets after human approval.
...
Git + GitHub *are* the audit log, by construction: every spec, task, commit, review comment, gate result, approval, and merge is timestamped and attributed (agent commits are labeled as such via distinct author identity). Weekly metrics snapshots ([05-metrics.md](05-metrics.md)) go in-repo. Answering "who decided X and why" must never require an archaeology project — if it does, the trail has a gap to fix.
