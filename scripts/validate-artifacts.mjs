...
for (const file of listMarkdown('specs')) {
  const match = SPEC_FILE.exec(file);
  if (!match) {
    fail(`specs/${file}`, 'filename must match SPEC-<nnn>-<kebab-slug>.md');
    continue;
  }
  const body = readFileSync(join(root, 'specs', file), 'utf8');
  const status = field(body, 'Status');

  if (!status) fail(`specs/${file}`, 'missing a **Status:** line');
  else if (!SPEC_STATUSES.includes(status))
    fail(`specs/${file}`, `status "${status}" is not one of ${SPEC_STATUSES.join(' | ')}`);

  for (const section of ['## Open questions & assumptions', '## Acceptance criteria']) {
    if (!body.includes(section)) fail(`specs/${file}`, `missing required section "${section}"`);
  }

  if (specs.has(match[1]))
    fail(`specs/${file}`, `duplicate spec number ${match[1]} (also ${specs.get(match[1]).file})`);
...
for (const file of listMarkdown('tasks')) {
  const match = TASK_FILE.exec(file);
  if (!match) {
    fail(`tasks/${file}`, 'filename must match TASK-<nnn>.<n>-<kebab-slug>.md');
    continue;
  }
  const [, specNum] = match;
  const body = readFileSync(join(root, 'tasks', file), 'utf8');
  const status = field(body, 'Status');
  const specRef = field(body, 'Spec');
  const branch = field(body, 'Branch');

  if (!status) fail(`tasks/${file}`, 'missing a **Status:** line');
  else if (!TASK_STATUSES.includes(status))
    fail(`tasks/${file}`, `status "${status}" is not one of ${TASK_STATUSES.join(' | ')}`);

  if (!body.includes('## Done-criteria'))
    fail(`tasks/${file}`, 'missing required section "## Done-criteria"');

  // Spec 000 is the reserved "no spec" bucket for maintenance work.
  if (specNum !== '000') {
    const spec = specs.get(specNum);
    if (!spec) {
      fail(`tasks/${file}`, `references SPEC-${specNum}, which does not exist`);
    } else {
      if (spec.status === 'Draft')
        fail(`tasks/${file}`, `derives from SPEC-${specNum}, which is still Draft — only a human may approve a spec before tasks are written`);
      (tasksBySpec.get(specNum) ?? tasksBySpec.set(specNum, []).get(specNum)).push({ file, status });
    }
  } else if (specRef && !/none/i.test(specRef)) {
    fail(`tasks/${file}`, 'TASK-000.* is the no-spec bucket; its **Spec:** line should say "none"');
  }

  if (branch && !/^(task\/\d{3}\.\d+-[a-z0-9-]+|none)/.test(branch))
    fail(`tasks/${file}`, `branch "${branch}" must match task/<specnnn>.<n>-<slug>`);
}
...
for (const [num, spec] of specs) {
  ...
  for (const task of open)
    fail(`specs/${spec.file}`, `marked Implemented but ${task.file} is still "${task.status}"`);
}
